...
class Sku:   
    ...
    def copy_images(self):
        for sku in self.sku_list:
            try:
                ...
                original = glob.glob(f'{self.path1}\\**\\{sku["SKU"]}.*', recursive=True)
                target = f'{self.path2}\\{sku["SKU"]}.png'
    ...
    def add_stamp(self):
        ...
        missing_file = []
        for sku in self.sku_list: 
            original = glob.glob(f'{self.path2}\\{sku["SKU"]}.png')   
            #if there is no file in the path, add the name of the file in the missing_file                     
            if original == []:
                missing_file.append(sku["SKU"])                
                pass
            
            else:
                image1 = Image.open(f'{self.path2}\\{sku["SKU"]}.png')
                ...
                image1.save(f'{self.path2}\\{sku["SKU"]}.png')
